...
class TestPaginatedStream(unittest.TestCase):
    ...
    def test_page_number_increments(self, mock_req):
        ...
        s = PaginatedStream(stream_id="t", path="t")
        list(s.fetch_data(SurveyMonkeyClient("tok"), None, {}, {}))
        # The params dict is mutated in place, so all call records reference the
        # same dict (page=3 by the end).  What matters is that 3 API calls were
        # made and that each used a 'page' param.
